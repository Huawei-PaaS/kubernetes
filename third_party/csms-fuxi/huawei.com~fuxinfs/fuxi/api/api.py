...
class API(object):
    ...
    def create(self, params):
        ...
        ok, result = volume_validator.validate_share_proto(params)
        if not ok:
            self.AUDIT_LOG.info("Create volume failed")
            return result
        LOG.info(_LI("share_proto:  " + result))

        ok, result = volume_validator.validate_access_to(params)
        if not ok:
            self.AUDIT_LOG.info("Create volume failed")
            return result
        LOG.info(_LI("access_to:  " + result))

        ok, result = volume_validator.validate_access_level(params)
        if not ok:
            self.AUDIT_LOG.info("Create volume failed")
            return result
        LOG.info(_LI("access_level:  " + result))
    ...
    def expand(self, new_size, old_size, params):
        ...
        self.AUDIT_LOG.warn("Expand sfs share start")

        ok, result = common_validator.validate_namespace(params)
        if not ok:
            self.AUDIT_LOG.warn("Expand sfs share failed")
            return result
        namespace = result
        ...
        if message_code == consts.EXPAND_OK:
            self.AUDIT_LOG.warn("Expand sfs share success")
            return response.create_success_response()
        else:
            self.AUDIT_LOG.warn("Expand sfs share failed")
            return response.create_error_response(msg, message_code)

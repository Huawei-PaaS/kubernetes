...
def main(argv):
    ...
    if cmd not in ["create", "delete", "attach", "isattached", "detach", "mount", "mountdevice",
                   "unmount", "unmountdevice", "init", "waitforattach", "expandvolume", "getvolumelimitinfo", "getvolumelimitkey"]:
        error_message = "{\"status\": \"Failed\", \"message\": \"Invalid usage.\"}"
        print(error_message)
        return
    else:
...
def get_request_body(args, cmd, exec_cmd_list):
    request_body = ''
    if "attach" == cmd:
        if len(args) < 3 or args[-1] == "None":
            return False, None
        request_body = get_request_option(args[1:len(args) - 1])
...
def check_request_parameters(args, cmd):
    if "mount" == cmd or "mountdevice" == cmd:

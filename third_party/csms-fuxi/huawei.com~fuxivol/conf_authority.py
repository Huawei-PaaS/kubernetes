...
def get_log_path():
    log_path = "/usr/libexec/kubernetes/kubelet-plugins/volume/exec/huawei.com~fuxivol/etc/fuxi.conf"
...
def authority_config(install_path):
    try:
        ...
        os.chmod('/etc/logrotate.d/fuxi.logrotate.conf', stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP)
    ...
    try:
        ...
        if len(path_list) == 0:
            ...
            for temp_list in path_list:
                ...
                if (subdirectory.endswith("xml") and os.path.isfile(subdirectory)) or subdirectory.endswith(
                        "mkfs") or subdirectory.endswith("unmount") or subdirectory.endswith("mount"):

...
node_conf_path = '/var/paas/conf/node.conf'
LOG = logging.getLogger(__name__)

def get_real_host_type():
    if os.path.exists(node_conf_path):
        try:
            node_conf_file = open(node_conf_path, "r")
            node_conf_dict = json.load(node_conf_file)
            return node_conf_dict["type"]
        except Exception as e:
            LOG.error(_LE("Failed to get real node type from %s: %s" % (node_conf_path, e.message)))
            return None
        finally:
            node_conf_file.close()
    else:
        LOG.warning(_LW("Failed to get real node type: %s not exist" % (node_conf_path)))
        return None
...
def get_keystone_session():
    ...
    cfg['auth_url'] = config.get("CINDER", "auth_url", None)  # mandatory
    cfg['username'] = config.get("CINDER", "username", None)  # used for Password method:v3.Password

...
func SetupQuotaforCreateAndUpdate(usedCPU string, usedMemory string, stopCh chan struct{}) (*fake.Clientset, *QuotaAdmission) {
	resourceQuota := &api.ResourceQuota{
...
func TestAdmitBelowQuotaLimitWithDecreasingResourceUpdate(t *testing.T) {
	...
	stopCh := make(chan struct{})
	defer close(stopCh)
	kubeClient, handler := SetupQuotaforCreateAndUpdate("1", "50Gi", stopCh)
	...
	kubeClient, handler = SetupQuotaforCreateAndUpdate("1200m", "60Gi", stopCh)
...
func TestAdmitBelowQuotaLimitWithIncreasingResourceUpdate(t *testing.T) {
	...
	kubeClient, handler = SetupQuotaforCreateAndUpdate("1100m", "52Gi", stopCh)
...
func TestAdmitExceedsQuotaLimitWithUpdate(t *testing.T) {
	...
	// update the usage to account for the first update
	_, handler = SetupQuotaforCreateAndUpdate("1100m", "52Gi", stopCh)
